...
from numpy import argsort, dot, linalg, zeros
...
OPENROUTER_API_KEY = os.environ.get(
    "OPENROUTER_API_KEY_KIMIE2",
    "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",
)

ASTRA_DB_ENDPOINT = os.environ.get("ASTRA_DB_ENDPOINT")
ASTRA_DB_TOKEN = os.environ.get("ASTRA_DB_TOKEN")
OPENROUTER_API_KEYROTEIRO_DISP_KIMIE_K2FREE = os.environ.get(
    "OPENROUTER_API_KEYROTEIRO_DISP_KIMIE_K2FREE"
)

logger.info(f"ASTRA_DB_ENDPOINT set: {bool(ASTRA_DB_ENDPOINT)}")
logger.info(f"ASTRA_DB_TOKEN set: {bool(ASTRA_DB_TOKEN)}")
logger.info(
    f"OPENROUTER_API_KEYROTEIRO_DISP_KIMIE_K2FREE set: {bool(OPENROUTER_API_KEYROTEIRO_DISP_KIMIE_K2FREE)}"
)
    ...
    def load_pdf_content(self):
        ...
        try:
            if os.path.exists(self.pdf_path):
                ...
                self.chunks = chunk_text(self.pdf_text)
    ...
    def get_relevant_chunks(self, question, top_k=3):
        ...
        try:
            ...
            if keyword_chunks:
                ...
                relevant_chunks = [
                    selected_chunks[i] for i in top_indices if final_scores[i] > 0.05
                ]
        ...
        try:
            ...
            if confidence < 0.3:  # Threshold ajustado
                if personality_id == "dr_gasnelio":
                    ...
                    answer = (
                        "Olha, encontrei algumas informações relacionadas, mas não tenho certeza total. Vou te contar o que sei: "
                ...
                "body": json.dumps(
                    {
                        ...
                        "models_loaded": chatbot.qa_pipeline is not None
                        and chatbot.embedding_model is not None,
