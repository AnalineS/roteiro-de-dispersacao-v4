...
def create_app() -> Flask:
    ...
    def chat():
        ...
        try:
            ...
            use_langflow = data.get('use_langflow', False)
            ...
            # Gerar resposta
            if use_langflow:
                # Usar LangFlow para processamento avançado
                response = flow_manager.process_dispersion_query(message)
                if not response:
                    # Fallback para RAG se LangFlow falhar
                    response = rag_system.generate_response(message, persona, session_id)
            else:
                # Usar sistema RAG padrão
                response = rag_system.generate_response(message, persona, session_id)
    ...
    def upload_document():
        ...
        try:
            ...
            # Processar arquivo (implementar conforme necessário)
